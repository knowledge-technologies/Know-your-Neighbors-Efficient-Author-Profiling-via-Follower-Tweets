...
def export_labels(labels_file):
    ...
    with open(labels_file) as lf:
        for line in lf:
            ...
            labels['hundrets'].append(h)
            labels['years_clf'].append(augment_years(int(curr)))
            labels['tens'].append(days)
            #print("YEAR: "+str(curr)+" TENS: "+str(days)+"HUNDERTS: "+str(h))
...
def build_year(x,y):
    return 1900 + int(x)*10 + y;
def fit(path,out_path="../out"):
    tic()
    test_texts = {}
    inv_g = {0:'male',1:'female'}
    inv_o = {0:'sports',1:'performer',2:'creator',3:'politics'}
    tokenizer,clfs,reducer = fit_import("/home/koloski20/ff_splits_all/train_data")
    """
    ...
    hundrets = clfs['hundrets'].predict(reduced_matrix_form)
    tens = clfs['tens'].predict(reduced_matrix_form)    
    cnt = 0
    for x in test_texts:                 
        o = inv_o[int(occupation[cnt])]
        g = inv_g[int(gender[cnt])]        
        y = reverse_augment(augment_years(build_year(hundrets[cnt],tens[cnt])))
