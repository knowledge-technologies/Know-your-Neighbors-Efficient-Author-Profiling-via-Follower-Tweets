...
"""     
def reverse_augment(in_): 
    ins = [ int((1949+1958)/2), #0 1958
            int((1959+1966)/2), #1 1959,1966
            int((1967+1973)/2), #2 1967,1973
            int((1974+1980)/2), #3 1974,1980
            int((1981+1986)/2), #4 1981,1986
            int((1987+1991)/2), #5 1987,1991
            int((1992+1995)/2), #6 1992,1995
            int((1996+1999)/2)  #7 1996
            ]
    return ins[in_] 
...
def fit(path,out_path="../out"):
    ...
    tokenizer,clfs,reducer = fit_import("/home/koloski20/ff_clf_avg/train_data")
    """
